...
class FamilyCalState extends ChangeNotifier {
  ...
  })  : _members = List.of(members), // MUTABLE internally
  ...
  CalendarViewMode _calendarViewMode = CalendarViewMode.month; // default Month
  ...
  void selectCalendarDay(DateTime date) {
    ...
    if (normalized == _selectedCalendarDay) return;
  ...
  void setCalendarViewMode(CalendarViewMode mode) {
    if (_calendarViewMode == mode) return;
  ...
  void setVisibleMonth(DateTime month) {
    ...
    if (_visibleMonth.year == normalized.year && _visibleMonth.month == normalized.month) return;
  ...
  List<EventInstance> nowAndNextInstances(DateTime now) {
    ...
    final upcoming = todayInstances.where((instance) {
      if (isConfirmed(instance)) return false;
  ...
  List<EventInstance> agendaFor(DateTime anchorDay) {
    ...
    final items = [...instancesForDay(today), ...instancesForDay(tomorrow)]
      ..sort((a, b) => a.windowStart.compareTo(b.windowStart));
    return items;
  }

  bool instanceIsActive(EventInstance instance, DateTime now) =>
      !now.isBefore(instance.windowStart) && !now.isAfter(instance.windowEnd);

  bool isConfirmed(EventInstance instance) => findConfirmation(instance) != null;
  ...
  void confirmOccurrence(
    ...
    if (isConfirmed(instance)) return;
  ...
  void recordMissed(EventInstance instance) {
    ...
    final log = ConfirmationLog(
      ...
      responsibleMemberId: instance.event.responsibleMemberId,
      windowStart: instance.windowStart,
      confirmedById: instance.event.responsibleMemberId ?? _currentMemberId,
  ...
  void switchCurrentMember(String memberId) {
    if (memberId == _currentMemberId) return;
    _currentMemberId = memberId;
    notifyListeners();
  }

  FamilyChild childById(String id) => _children.firstWhere((c) => c.id == id);
  FamilyPlace placeById(String id) => _places.firstWhere((p) => p.id == id);
  FamilyMember memberById(String id) => _members.firstWhere((m) => m.id == id);
  ...
  void addMember(FamilyMember member) {
    _members.add(member); // mutate instead of reassigning an unmodifiable list
    notifyListeners();
  }
  ...
  List<ConfirmationLog> logsForChildAndMonth(
    ...
    return _confirmations.where((log) {
      if (childId != null && log.childId != childId) return false;
